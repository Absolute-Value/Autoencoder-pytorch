...
from modules.ae import AutoEncoder
...
net = AutoEncoder(in_c=img_ch, hidden_dim=args.dim).to(device)
...
with torch.no_grad():
    ...
    for i, (data_t, label) in enumerate(test_loop):
        ...
        if flag[label[0]] < 10 or args.dataset == 'cloth':
            utils.save_image(data_t, args.adpic_dir + '/test{}_input.png'.format(i), normalize=True)
            utils.save_image(outputs.data, args.adpic_dir + '/test{}_output.png'.format(i), normalize=True)
            utils.save_image(outputs.data.cpu()-data_t, args.adpic_dir + '/test{}_diff{}_{:.4f}.png'.format(i, label[0], loss.item()))
...
with open('result/result.csv', 'a', newline="") as f:
    writer = csv.writer(f)
    writer.writerow([args.dataset, args.data_type, args.img_size, args.gray, args.epochs, args.dim, args.lr, args.init_type, args.batch, args.seed, auc])

with open(args.save_dir + '/test.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['data', 'class', 'size', 'gray', 'Epoch', 'dim', 'lr', 'init', 'batch', 'seed', 'AUC'])
    writer.writerow([args.dataset, args.data_type, args.img_size, args.gray, args.epochs, args.dim, args.lr, args.init_type, args.batch, args.seed, auc])
