...
parser = argparse.ArgumentParser(description='AE_AD')
...
result_dir = 'result/{}/{}_{}Ep{}lr{}{}Seed{}_'.format(args.dataset, args.data_type, args.img_size, args.epochs, args.lr, args.init_type, args.seed) + dt_now.strftime('%Y%m%d_%H%M%S')
if args.gray == 'TRUE':
    result_dir += '_gray'
    img_ch = 1
else:
...
for epoch in range(1, args.epochs + 1):
    ...
    if epoch % 10 == 0:
        with torch.no_grad():
            ...
            utils.save_image(vis_outputs[:25], pic_dir + '/validation-{}.png'.format(epoch), nrow=5, normalize=True)
            '''
...
with open(result_dir + '/train.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['data', 'class', 'size', 'gray', 'Epoch', 'dim', 'lr', 'init', 'batch', 'seed', 'loss'])
    writer.writerow([args.dataset, args.data_type, args.img_size, args.gray, args.epochs, args.dim, args.lr, args.init_type, args.batch, args.seed, train_loss_list[-1]])
